"""v13 시간대 차단 제거 검증 — CLAUDE.md "제거 시 -99%" 주장이 v13에서 맞나.
원본 vs 무차단 vs ±시프트 vs 랜덤, 전체+연도별."""
...
BASE = "config/final_v13_eth.yaml"
...
def main():
    ...
    print("=== v13 시간대 차단 강건성 (전체+연도별, $100→ / Sharpe / MDD) ===\n")
    print(f"{'변형':10} {'full$':>12} {'Sh':>5} {'MDD%':>6} | {'22$':>7} {'23$':>7} {'24$':>7} {'25$':>8}")
    for lbl in variants:
        f, sh, mdd = R[lbl]["full"]
        ys = {p: R[lbl][p][0] for p in ["2022", "2023", "2024", "2025"]}
        print(f"{lbl:10} {f:>12,.0f} {sh:>5.2f} {mdd:>6.0f} | {ys['2022']:>7,.0f} {ys['2023']:>7,.0f} "
