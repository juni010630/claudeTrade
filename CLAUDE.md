...
- **백테는 항상 라이브 종목수(풀 유니버스)로** — 소규모 서브셋은 상관-집중 tail risk를 은폐. (2026-06-13 검증: momentum_breakout 20종목 Sharpe~2.0 → 83종목 MDD -112% 파산. 서브셋 결과로 사이징/채택 금지.)
...
### 설정 파일
- `config/final_v21d_eexit.yaml` — **현 라이브 (2026-06-19~)** = v21c + 반대신호 조기청산. 계보: v18→v19(DVOL 변동성타게팅)→v20(per-book DVOL t50)→v21b(볼륨필터 multi vol2.0+scorer vol_thr2.0)→v21c(+corr_lookback150)→**v21d**(+`early_exit_on_opp`: ema_cross·multi_tf가 반대신호 시 조기청산 — 엔진 `_process_bar` 4.5단계 `check_early_exit`, 627–632행). 83심볼 트리플(추세+MR슬리브+macross_d 격리북), deep_floor -0.55, tp/sl_reversal off. 상세=memory [[project_screw_sweep_v20]]·[[project_dev_stop_v21c]]·repo NEWEDGE_GREEDY/SCREW MD. **2026-06-19 코드 전수 버그리뷰 14건 수정·배포** (memory [[project_bug_review_fixes_2026_06_19]]; 캐시 데이터갭 04-22~06-08 = [[project_data_gap_2026_06_19]]). edge_monitor는 v21 베이스라인(`config/edge_baseline_v21c.json`)으로 마이그 필요(ANCHOR/CONFIG 갱신). **2026-06-20 재검토 6건 수정·배포(커밋 124f610, [[project_bug_review_2026_06_20]])**: #1 multi_tf 조기청산이 배포 이래 NameError로 죽어있던 것(=multi 북은 v21c와 동일동작) 부활 — 풀유니버스 검증 +8.2%·MDD동일·2024+14pp; +maker 나체포지션 가드·hammer defensive 대칭배선·청산 stale 이월손실·loader 갭 윈도 클램프. edge_monitor는 이미 v21d 정렬됨(CONFIG/ANCHOR=06-19 23:00 보정·배포완료, baseline만 31일 누적 후 생성 — 위 '마이그 필요'는 해소). **2026-07-04 코드 정밀감사 수정 배포**: CRITICAL 1(fetch_open_symbols 실패=빈집합 오판→가짜청산·SL취소·중복진입)·HIGH 2(maker 취소실패 2배포지션 / 빈 1h프레임→4h폴백 오청산) 포함 그룹A 견고성 18건(백테 비트동일 검증) + 그룹B 4건(상관필터 timestamp 정렬·FILUSDT macross_d 부활·CB 정지앵커=손실시점·상폐 타임아웃). **honest 베이스라인 $11,342→$11,263(-0.7%, CB앵커 스펙정합 비용·연도별 균등)/Sh2.429/일별MDD-26.3%/1075거래.** B1·B2·B4는 구간 내 비트동일. edge_monitor ANCHOR=07-03 18:00 리셋(구엔진 이력 패리티 오염 방지, baseline 누적 재시작). 상세=memory [[project_code_audit_2026_07_04]]. **2026-07-04 2차 리뷰 수정 배포(커밋 51e750d, [[project_code_review_2026_07_04b]])**: maker 이중진입 가드 실효화(HIGH)+청산실패 TP/SL 재등록+multi_tf 1d필터 fail-closed — 라이브 전용, 백테 무영향. **2026-07-05 펀딩 정산 ts 전진 감지 채택**: 전역 8h 버킷 → 심볼별 정산 타임스탬프 기반(바이낸스 4h 정산 전환 17심볼 반영, DH2 위상 해소 포함, 라이브=history API ts 동일 의미론·실패 시 8h 버킷 폴백). **honest 베이스라인 $11,263→$11,275(+0.1%)/Sh2.429 동일/일별MDD-26.3% 동일/1075거래 동일/일수익률 상관 0.999998** = 의도한 펀딩 차이만 발생, 경로 불변.
- `config/final_v18_triple.yaml` — (이전 라이브 2026-06-11~06-17) = v17 + macross_d(1d EMA20×100 알트78 숏수확 슬리브, 격리 북) **40:30:30** 배분. 백테 ×45.7/Sh일 1.86/**MDD일 -30.8%**(v17 -42.2%)/2023 +11%. 검증·프론티어 = `NEWEDGE_GREEDY_RESULTS.md`.
...
### 설정 파일
- `config/final_v21d_eexit.yaml` — **현 라이브** (v21c + 반대신호 조기청산, 2026-06-19 배포)
- `config/final_v13_eth.yaml` ~ `final_v18_triple.yaml`, `final_v19_dvol.yaml`, `final_v20_dvol_perbook.yaml`, `final_v21{,b,c}_*.yaml`, `merged_v16_sleeve.yaml` — 버전 계보 (상단 "최종 전략" 섹션 참조)
