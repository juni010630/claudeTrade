"""v16:슬리브 자본배분 곡선 (90:10 ~ 50:50) — 수익 vs MDD trade-off."""
...
def totret(r): return ((1+r).prod()-1)*100
...
def main():
    ...
    print("=== v16:슬리브 자본배분 (일별 리밸런스, 2022~2026) ===")
    print(f"{'배분(v16:슬)':>12} | {'총수익%':>12} {'CAGR%':>7} {'Sharpe':>6} {'MDD%':>6} {'최종$100→':>10}")
    for w16 in [100, 90, 80, 70, 60, 50]:
        w = w16/100
        r = w*j.v16 + (1-w)*j.sl
        eq = (1+r).cumprod().iloc[-1]*100
        years = len(r)/365
        cagr = ((eq/100)**(1/years)-1)*100
        tag = "(v16 단독)" if w16 == 100 else ""
        print(f"{w16:>4}:{100-w16:<3}     | {totret(r):>12,.0f} {cagr:>7.0f} {sharpe(r):>6.2f} {mdd(r):>6.0f} {eq:>10,.0f} {tag}")
