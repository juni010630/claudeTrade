...
def test_first_run_seeds_state_without_realerting_existing_issue(tmp_path):
...
def test_resolved_alert_can_notify_on_recurrence(tmp_path):
    path = tmp_path / "edge_alert_state.json"
    should_notify_alert_transition({"parity:old"}, state_path=path)
    assert not should_notify_alert_transition(set(), state_path=path)
