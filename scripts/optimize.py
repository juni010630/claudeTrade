...
Usage:
    python scripts/optimize.py --trials 30 --metric sharpe
    python scripts/optimize.py --trials 50 --metric total_return
    python scripts/optimize.py --resume          # 이전 결과 이어서

최적화 목표 (--metric):
  sharpe        Sharpe Ratio (기본값 — 위험조정수익률)
  total_return  총 수익률
  calmar        Calmar Ratio (수익률 / MDD)
"""
...
SEARCH_SPACE: dict[str, Any] = {
    ...
    "risk.max_positions":           [3, 4, 5],
    "risk.daily_drawdown_pause":    [-0.04, -0.05, -0.06],
    "risk.daily_drawdown_stop":     [-0.07, -0.08, -0.10],

    # 레버리지 티어
    "leverage_tiers.S.leverage":    [10, 12, 15, 20],
    "leverage_tiers.S.size_fraction": [0.20, 0.25, 0.30, 0.35],
    "leverage_tiers.A.leverage":    [5, 7, 10],
    "leverage_tiers.A.size_fraction": [0.15, 0.20, 0.25],
    ...
    "regime.bb_width_squeeze_pct":   [0.15, 0.20, 0.25],

    # 스코어러
    "scorer.volume_ratio_threshold": [1.2, 1.5, 1.8, 2.0],
    "scorer.rsi_long_max":           [60.0, 65.0, 70.0],
    "scorer.rsi_short_min":          [30.0, 35.0, 40.0],
    "scorer.tier_s_min_score":       [5, 6, 7],
    "scorer.tier_a_min_score":       [3, 4, 5],
    "scorer.tier_b_min_score":       [2, 3],

    # momentum_breakout
    "strategies.momentum_breakout.bb_std":          [1.8, 2.0, 2.2, 2.5],
    "strategies.momentum_breakout.atr_tp_mult":     [1.5, 2.0, 2.5, 3.0],
    "strategies.momentum_breakout.atr_sl_mult":     [0.7, 0.8, 1.0, 1.2],
    "strategies.momentum_breakout.volume_multiplier": [1.2, 1.5, 1.8, 2.0],

    # ema_cross
    "strategies.ema_cross.fast_period":   [7, 9, 12],
    "strategies.ema_cross.slow_period":   [18, 21, 26],
    "strategies.ema_cross.atr_tp_mult":   [2.0, 2.5, 3.0, 3.5],
    "strategies.ema_cross.atr_sl_mult":   [0.7, 0.8, 1.0, 1.2],

    # mean_reversion
    "strategies.mean_reversion.bb_std":          [1.8, 2.0, 2.2],
    "strategies.mean_reversion.rsi_oversold":    [25.0, 30.0, 35.0],
    "strategies.mean_reversion.rsi_overbought":  [65.0, 70.0, 75.0],
    "strategies.mean_reversion.atr_tp_mult":     [1.2, 1.5, 2.0],
    "strategies.mean_reversion.atr_sl_mult":     [0.7, 0.8, 1.0],
}

RESULTS_FILE = Path("optimize_results.json")
...
def score_report(report, metric: str) -> float:
    mdd = report.max_drawdown  # 음수 퍼센트 (예: -20.5)
    trades = report.total_trades

    # 공통 필터: 거래 수 너무 적으면 노이즈
    if trades < 10:
        return -999.0

    if metric == "sharpe":
        # MDD -35% 초과 시 하드 거부 (크립토 기준 상한)
        if mdd < -35.0:
            return -999.0
...
def main() -> None:
    ...
    parser.add_argument("--trials", type=int, default=20, help="이번 실행에서 시도할 횟수")
    ...
    parser.add_argument("--resume", action="store_true", help="이전 결과 이어서")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--apply-best", action="store_true", help="최적 파라미터를 params.yaml에 저장")
    ...
    print(f"최적화 시작 — 목표 지표: {args.metric.upper()}, 시도 횟수: {args.trials}")
    if results:
        best_so_far = max(results, key=lambda r: r["score"])
        print(f"이전 최고 점수: {best_so_far['score']:.4f}  "
              f"(총 수익률 {best_so_far['summary']['total_return']:+.1f}%,  "
              f"Sharpe {best_so_far['summary']['sharpe']:.3f},  "
              f"MDD {best_so_far['summary']['max_drawdown']:.1f}%)")
    print()
    ...
    print("\n" + "=" * 60)
    print(f"  최적 결과 (Trial #{best['trial']})")
    print("=" * 60)
    bs = best["summary"]
...
def _extract_diff(base: dict, new: dict, prefix: str = "") -> dict[str, tuple]:
    diff = {}
