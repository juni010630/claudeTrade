...
### 설정 파일
- `config/final_v18_triple.yaml` — **현 라이브 (2026-06-11~, 커밋 724cd8d)** = v17 + macross_d(1d EMA20×100 알트78 숏수확 슬리브, 격리 북) **40:30:30** 배분. 백테 ×45.7/Sh일 1.86/**MDD일 -30.8%**(v17 -42.2%)/2023 +11%. 검증·프론티어 = `NEWEDGE_GREEDY_RESULTS.md`. edge_monitor v18 마이그레이션 완료(2026-06-13): CONFIG=v18·ANCHOR=06-12·ANCHOR_CAPITAL 자동조회. **배포 시** `config/edge_baseline_v18.json` 동봉 + 서버 첫 실행이 edge_cache 83심볼 자동수집(첫 실행만 느림). (06-13 '라이브전용 DYDXUSDT macross_d' WARN = 마이그레이션 전 v17/v18 불일치 오경보였음.)
