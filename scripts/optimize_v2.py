"""V2 스윙 전략 파라미터 최적화 (momentum_breakout + multi_tf_breakout + mean_reversion).

Usage:
    python scripts/optimize_v2.py --trials 30 --metric sharpe
    python scripts/optimize_v2.py --resume --trials 20
    python scripts/optimize_v2.py --apply-best
"""
...
SEARCH_SPACE: dict[str, Any] = {
    # 리스크
    "risk.risk_per_trade":          [0.005, 0.008, 0.01, 0.012, 0.015],
    ...
    # 레버리지 티어
    "leverage_tiers.S.leverage":      [8, 10, 12, 15],
    ...
    "scorer.rsi_long_max":           [55.0, 60.0, 65.0, 70.0],
    "scorer.rsi_short_min":          [30.0, 35.0, 40.0, 45.0],
...
RESULTS_FILE = Path("optimize_v2_results.json")
...
def main() -> None:
    ...
    print(f"V2 최적화 시작 — 목표 지표: {args.metric.upper()}, 시도 횟수: {args.trials}")
    if results:
    ...
    print("\n" + "=" * 60)
    print(f"  V2 최적 결과 (Trial #{best['trial']})")
    print("=" * 60)
    bs = best["summary"]
