...
### 설정 파일
- `config/merged_v16_sleeve.yaml` — **채택 시스템 (2026-06-09, 백테 검증 / 라이브 검증 대기)** = 단일계좌에 v16(funding3)+슬리브 평균회귀 공존. per-strategy 레버(슬리브 flat lev3) + capital_fraction 5:5 + daily DD stop off. 백테 $4,442/Sh1.64(daily)/MDD-45%(daily)·-49.7%(bar)/PF1.51. ⚠️ 11심볼+1d 평균회귀가 1h 라이브서 미검증 — dry-run 후 배포. [[project_v16_overfit_audit]]
