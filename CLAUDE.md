...
### 설정 파일
- `config/final_v17.yaml` — **현 라이브 (2026-06-09~, 메인넷 systemd `trade-bot.service`, 봇 운용자본 ~$9,450 — 2026-06-10 사용자 시드 충전, 정상)** = merged_noblock(시간차단 funding3 제거 + 슬리브 50:50) + **SS6+sd4**(tier_ss_min_score 5→6, max_same_direction 8→4). IS/OOS 홀드아웃 통과 — 직전 baseline을 IS·OOS 양 기간 파레토 지배(전구간 봉기준 $8,991/Sh1.94/MDD봉-42.4%/805거래; OOS 2025~ MDD-35.5/Sh3.44). vol_thr 2.2는 과적합으로 폐기. ⚠️ daily DD stop OFF. (봉MDD 갭 SL 낙관 우려는 2026-06-10 실측 0건으로 해소 — `GAP_SL_RESULTS.md`.) 봇 운용자본: 초기 $2,451(시드 $7,451 중 $5K spot 분리) → **2026-06-10 충전 후 ~$9,450**. 봇 제어=`sudo systemctl restart trade-bot`만(수동실행=중복인스턴스 금지). 직전 라이브 merged_noblock_sleeve=아래 표 베이스. [[project_oat_ablation_2026_06]] [[project_remote_deploy]] [[user_funding_caution]]
